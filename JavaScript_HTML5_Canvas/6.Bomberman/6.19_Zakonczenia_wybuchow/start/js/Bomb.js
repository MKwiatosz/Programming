...
Bomb.prototype.draw = function() {
	...
	if(this.timer>0){
		...
		this.bum_type = 'center_bum';// ustaw rodzaj wybuchu
		...
		// Wrzuć na planszę wybuchy
		// Centralny już jest trzeba przeszukać 4 możliwości
		this.bums = [];// tablica z polami na których jest wybuch
		for (var i=0; i<4; i++) {// cztery możliwości (góra, dół, prawo, lewo)
			// czy x czy y
			this.axis = i%2 ? 'tmp_column' : 'tmp_row';
			// czy rośnie czy maleje
			this.grow = i%3 ? true : false;
			// wartości tmp będą zmieniane wewnątrz pętli
			...
			for (var j=0; j<this.range; j++) {// wybuch może zająć pola w promieniu wartości właściwości this.range
				// Tutaj zmienia się tmp_x lub tmp_y (przechowywane w this.axis)
				this[ this.axis ] = this[ this.axis ]+(this.grow ? 1 : -1);
				//
				if(Game.board.b[ this.tmp_row ][ this.tmp_column ].type!='solid'){// jeśli w danym miejscu jest podłoga lub skrzynka
					...
					// Jeśli to była skrzynka
					if(this.tmp_crate){
						break;//przerwij dalszą penetrację tego kierunku
					}
				}else if (Game.board.b[ this.tmp_row ][ this.tmp_column ].sub_type=='bomb' && !Game.board.b[ this.tmp_row ][ this.tmp_column ].bym_type)
				{
					Game.board.b[ this.tmp_row ][ this.tmp_column ].timer =0;
				
				}else{
					// Jeśli trafiło na ścianę przerwij dalsze penetrowanie tego kierunku
					break;
				}
			}
		}
