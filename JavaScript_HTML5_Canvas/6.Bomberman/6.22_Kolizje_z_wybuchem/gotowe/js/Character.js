...
Character.prototype.draw = function(){
	...
	if(this.change_f_delay<this.f_max_delay){
		...
		this.change_f_delay = 0;
		//  jeśli stan równa się „ko” to nie resetuj aktualnego numeru klatki tylko wywołaj metodę afterKO
...
// Definiuję rodzica dla Hero (podobnie jak w poprzednim filmiku definiowałem dla Enemy)
...
Hero.prototype.setKO = function(){
	...
	// Gracz nie może sterować postacią
	Game.stop();
}
// Nadpisuję metodę afterKO (jeśli nie wywołam metody afterKO rodzica to nie rozszerzam a nadpisuję metodę)
Hero.prototype.afterKO = function(){
	// Tutaj możemy wyświetlić gameOver albo wstawić nową postać albo cokolwiek.
	// Ja jedynie wrzucam do konsoli „gameOver”
	if(!Game.is_over){
		Game.is_over = true;
...
Enemy.prototype.afterKO = function(){
	...
	// teraz mogę sprawdzić czy został jakiś kurczak i zareagować jak wszycy będą zniszczeni
	var some_enemy = false
	for(var e in Enemy.all){
		some_enemy ++;
		// Jeśli pozostał jakiś kurczak to nie trzeba sprawdzać kolejnych
		break;
	}
	// Jeśli some_enamy równa się zero to gracz wygrał
	if(!some_enemy){
		console.log('success')
	}
}
